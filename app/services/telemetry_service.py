...
def parked_without_fix(params: dict | None) -> bool:
    """Трекер САМ говорит, что стоит на месте, и координат в пакете нет.

    ⚠️ Это НЕ сбой GPS. Владелец 15.09.2026 прислал снимок: Т557ОС178 стоит на
    «Верном», связь свежая («1 мин»), место верное — а плашка «GPS сбой» и
    пунктир. По логам сервера: на стоянке трекер шлёт пакет раз в минуту с
    нулями вместо координат, `motion: 0` и `lastSatTime` 5–24 минуты назад —
    спутники на стоянке он не держит (экономит питание или не видит неба под
    навесом). Как только машина трогается, координаты возвращаются сами.

    Судим ТОЛЬКО по явному флагу `motion` от трекера. Скорость в пакете без
    координат ничего не значит — без спутников она всегда ноль, и по ней
    «стоит» пришлось бы говорить и про машину, которая едет под глушилкой.
    Нет флага — False, то есть прежнее строгое «GPS сбой».
    """
    if not params:
        return False
    motion = params.get("motion")
    if motion is None or isinstance(motion, bool):
        return motion is False
    try:
        return int(float(motion)) == 0
