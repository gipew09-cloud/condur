...
# In-memory лимитер POST /login: не больше N попыток с одного IP за окно.
...
_LOGIN_HITS: dict[str, _deque] = {}
_LOGIN_WINDOW_SEC = 300
_LOGIN_MAX_PER_WINDOW = 20


def _login_rate_ok(ip: str) -> bool:
    now = _time.monotonic()
    dq = _LOGIN_HITS.setdefault(ip, _deque())
...
async def _security_middleware(request: Request, call_next):
    ...
    # молча сломает вид — это отдельная задача с проверкой каждой страницы.
    response.headers.setdefault("X-Frame-Options", "DENY")            # кликджекинг
    response.headers.setdefault("X-Content-Type-Options", "nosniff")  # подмена типа
    response.headers.setdefault("Referrer-Policy", "strict-origin-when-cross-origin")
    if settings.cookie_secure:  # только на HTTPS (проде)
...
async def _viewer_telegram_id(request: Request, session: AsyncSession) -> int | None:
    """Telegram ID того, кто сейчас смотрит кабинет (по постоянной сессии)."""
...
async def current_owner(
    ...
    # Единственный путь — постоянная сессia (таблица web_sessions, в cookie
    # только случайный токен, в БД — его SHA-256). Прежний запасной вход по
    # 7-дневному JWT убран: он подписывался JWT_SECRET, а дефолт секрета лежал
    # в .env.example на GitHub — знающий его мог подделать вход. Все живые
    # входы давно на сессиях, JWT всё равно истёк (жил 7 дней).
    ws = await _session_from_request(request, session)
    if ws is None:
...
async def update_driver(
    ...
    driver.full_name = clean_user_text(full_name)
...
async def admins_add(
    ...
    # Лимит: не более 5 администраторов на один кабинет
    admin_count = (
        await session.execute(
            select(func.count(Admin.id)).where(Admin.owner_id == owner.id)
        )
    ).scalar_one() or 0
    if admin_count >= 5:
        raise HTTPException(status_code=400, detail="Максимум 5 администраторов на кабинет")

    # Проверяем в рамках ЭТОГО кабинета (не глобально — один человек
    # может быть админом у разных владельцев, это нормально)
    exists = (
        await session.execute(
            select(Admin).where(Admin.telegram_id == tid, Admin.owner_id == owner.id)
        )
    ).scalar_one_or_none()
    if exists is None:
        session.add(Admin(owner_id=owner.id, telegram_id=tid, name=clean_user_text(name)))
        try:
            # Аудит: кто и когда выдал доступ. Без этого утечка доступа не
            # оставляет следов (см. историю со взломом соседнего бота).
            actor = await _viewer_telegram_id(request, session)
            await log_event(
                session, owner_id=owner.id, event_type="admin_added",
                payload={"admin_telegram_id": tid, "by_telegram_id": actor},
            )
            await session.commit()
        except IntegrityError:
            await session.rollback()
...
async def admins_delete(
    ...
    # Аудит отзыва доступа.
    actor = await _viewer_telegram_id(request, session)
    await log_event(
        session, owner_id=owner.id, event_type="admin_removed",
        payload={"admin_telegram_id": admin.telegram_id, "by_telegram_id": actor},
    )
...
async def routes_page(
    ...
    renamed: Annotated[int | None, Query()] = None,
):
...
async def routes_rc_edit(
    ...
    if not name.strip() or not address.strip():
        raise HTTPException(status_code=400, detail="Укажите название и адрес РЦ")
    old_name = (center.name or "").strip()
    _apply_distribution_center_form(center, {
        "name": name, "address": address, "aliases": aliases,
        "latitude": latitude, "longitude": longitude,
        "geofence_radius_m": geofence_radius_m,
    })
    new_name = (center.name or "").strip()

    # Переименовали РЦ — тянем новое имя в ШАБЛОНЫ маршрутов. Иначе водитель
    # в боте продолжает видеть старое название («Магнит (Гранд-Трейд)» вместо
    # «Магнит Шушары 179») и путает три склада одной сети.
    # РЕЙСЫ НЕ ТРОГАЕМ НИКОГДА: это исторические факты, по ним печатаются акты
    # для налоговой — задним числом менять их нельзя.
    renamed = 0
    if old_name and new_name and old_name != new_name:
        for field in (RouteTemplate.destination, RouteTemplate.origin):
            res = await session.execute(
                update(RouteTemplate)
                .where(RouteTemplate.owner_id == owner.id, field == old_name)
                .values({field.key: new_name})
            )
            renamed += res.rowcount or 0
        # Название шаблона генерируется как «откуда → куда» — обновляем и его.
        await session.execute(
            update(RouteTemplate)
            .where(RouteTemplate.owner_id == owner.id)
            .values(name=func.concat(RouteTemplate.origin, " → ", RouteTemplate.destination))
        )
    try:
        await session.commit()
    ...
    return RedirectResponse(f"/routes?renamed={renamed}", status_code=303)
...
async def create_trip_manual(
    ...
    trip = Trip(
        ...
        status="completed", origin=clean_user_text(origin), destination=clean_user_text(destination),
...
async def trip_edit_route(
    ...
    def _clean(value: str) -> str | None:
        value = clean_user_text(value)[:500]
